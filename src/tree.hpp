...
public:
    tree(float len,
        ...
        twigs.reserve(n);
        auto ephi = sf::Vector2f{ (float)sin(angledeg * M_PI / 180), (float)cos(angledeg * M_PI / 180) },
             tphi = sf::Vector2f{ (float)cos(angledeg * M_PI / 180), -(float)sin(angledeg * M_PI / 180) };
        for (size_t i = 0; i <= n; i++) {
            auto totalangle = angledeg + da * ((float)i - (float)n / 2);
            auto tphip = sf::Vector2f{ (float)cos(totalangle * M_PI / 180), -(float)sin(totalangle * M_PI / 180) };
            auto footpoint =
              origin + len * (float)(1 - pow((float)i / (2 * n), 1.5)) * ephi + thick / 2 * tphi - t / 2 * tphip;
            twigs.emplace_back(
              l, totalangle, t, footpoint, maxlevels, level + 1, branches, length, thickness, angularspread);
        }
    }
